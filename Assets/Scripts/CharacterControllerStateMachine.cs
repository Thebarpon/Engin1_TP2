...
{
    [SerializeField]
    public Cinemachine.CinemachineVirtualCamera Cine_Camera;
    ...
    [field: SerializeField]
    public GameObject RPunchHitbox { get; private set; }
    ...
    {
        //Only update in gameplaystate
        if (GameManagerSM._Instance.GetState() is GameplayState)
        {
            base.Update();
    ...
    {
        EventManager.EnableRPunchHitbox();
        Debug.Log("Enable punch hitbox");
    }
    public void DisableRPunchHitbox()
    {
        EventManager.DisableRPunchHitbox();
        Debug.Log("Disable punch hitbox");
    }
